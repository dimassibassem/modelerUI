...
describe('Basic Testing', () => {
  ...
  it('renders process Name and description in left Sidebar after submitting', () => {
    ...
    cy.get(
      '#root > div > aside > div.flex.flex-nowrap.justify-center.text-md.font-medium.leading-6.text-gray-900.sm\\:pt-1\\.5.items-center.truncate.outline-none.focus\\:outline-none'
    ).should('contain', name)
    cy.get(
      '#root > div > aside > div.flex.flex-nowrap.justify-center.text-sm.font-small.leading-6.text-gray-600.sm\\:pt-1\\.5.items-center.truncate.outline-none.focus\\:outline-none'
