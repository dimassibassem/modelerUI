...
const ContextMenu = ({
  ...
  return (
    <Menu id={MENU_ID}>
      <Item id={ContextMenuItems.Copy} disabled={handleDisable(nodes?.filter((node: Node) => node.selected))}
            onClick={handleItemClick}>Copy</Item>
      <Item id={ContextMenuItems.Cut} disabled={handleDisable(nodes?.filter((node: Node) => node.selected))}
            onClick={handleItemClick}>Cut</Item>
