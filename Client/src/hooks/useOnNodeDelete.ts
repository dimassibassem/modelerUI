    ...
    if (chainRecovery) {
      setEdges(
        deleted.reduce((acc: Edge[], node: Node) => {
          ...
          const sourcesInfo = edges.filter(({ source }) => incomersEdgesIds.includes(source))
          const targetsInfo = edges.filter(({ target }) => outgoersEdgesIds.includes(target))
          ...
          const createdEdges = sourcesWithConnectedHandle.reduce((accum: Edge[], { id: sourceId, sourceHandle }) => [
              ...accum,
